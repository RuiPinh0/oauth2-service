...
jest.mock('@aws-sdk/client-kms', () => ({
  KMSClient: jest.fn().mockImplementation(() => ({
    send: jest.fn().mockResolvedValue({ Signature: Buffer.from('signed').toString('base64url') })
  })),
  SignCommand: jest.fn()
}));
...
describe('OAuth2 /oauth/token endpoint', () => {
  it('should return a token for valid credentials', async () => {
    const response = await request(app)
      ...
      .send('grant_type=client_credentials&client_id=demo-client&client_secret=demo-secret&scope=default');
...
console.log(response.body) 
  ...
  it('should return 400 for unsupported grant_type', async () => {
    const response = await request(app)
      .post('/oauth/token')
      .set('Content-Type', 'application/x-www-form-urlencoded')
      .send('grant_type=password&client_id=demo-client&client_secret=demo-secret');
